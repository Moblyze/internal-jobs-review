...
function Breadcrumbs({ items }) {
  return (
    <nav className="flex mb-6 min-w-0" aria-label="Breadcrumb">
      <ol className="inline-flex items-center space-x-1 md:space-x-3 min-w-0 flex-nowrap max-w-full">
        ...
        {items.map((item, index) => (
          <li key={index} className="min-w-0 flex-shrink">
            <div className="flex items-center min-w-0">
              ...
              {item.href ? (
                <Link
                  to={item.href}
                  className="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 truncate"
                >
                  {item.label}
                </Link>
              ) : (
