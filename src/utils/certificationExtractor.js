...
export function extractJobCertifications(job) {
  ...
  // Use pre-extracted certifications from index file if available
  // (generated by scripts/generate-index.js to avoid needing full descriptions)
  if (job.extractedCertifications && job.extractedCertifications.length > 0) {
    const result = job.extractedCertifications
    if (job.id) jobCertCache.set(job.id, result)
    return result
  }

  const certifications = new Set()

  // Extract from description (only available on full job data, not index)
  if (job.description) {
